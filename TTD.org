...
** Index Work improvements 
*** TODO "reordering" for speed. 
- processing on indexed result is much, much slower for NFS and server
  file systems. While it is very fast on SSDs. I am assuming this is
  a fact caused by HDD vs SSD. Sequential (i-node order?) access is
  fairly fast, but randomized access of files (in various points of
  i-nodes) can get slower. If this is really the case, sorting the
  "index-hit-model" based on i-node order (which conveniently,
  follows alphabetic order --- due to the fact that they unzipped in
  that way), will help. 
**** TODO IMPLEMENT THIS improvements! 
- only a few lines of carefully written code
**** TODO TEST IT
- on gillespie, with/without reordering. ($HIT_REORDERING) 
